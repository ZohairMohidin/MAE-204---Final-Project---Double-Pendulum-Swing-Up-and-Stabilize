%% MAE 200 Swing Up Double Pendulum
%dynamics
% x = [x th1 th2 xDot th1Dot th2Dot]
clc
clear
clf
T = 3;  %Time to get pendulum upright
l1 = 1;
l2 = .5;
mc = 10;
m1 = 1;
...
s.B=[0; 0; 0; 1; 0; 0]; s.Q=diag([1 1 1 1 1 1]); s.R=0; s.QT=diag([1 1 1 0 100000 10000]);
s.x0=[0; pi; pi; 0; 0; 0];
u_kFirst = zeros(301,1);    % First guess of input
[u_k,x_k]=Example_22_2(T,u_kFirst);     %u_k,x_k are values at each step (input,states)

%% comput Matrix of states and K
%u_k = optimal input to get pendulum upright
%x_k  = states at each time step


% initialize and compute Variables
...
% March differential Ricatti Equations to get X and P

for i=size(A,3):-1:2
    h=.01;
        f1 = RHS_DRE_K(X(:, :, i), E(:, :, i), A(:, :, i), B, R, Q);
        f2 = RHS_DRE_K(X(:, :, i)-f1/2*h, E(:, :, i), A(:, :, i), B, R, Q);
        f3 = RHS_DRE_K(X(:, :, i)-f2/2*h, E(:, :, i), A(:, :, i), B, R, Q);
        f4 = RHS_DRE_K(X(:, :, i)-f3*h, E(:, :, i), A(:, :, i), B, R, Q);
...
for i=1:size(A,3)-1
    h=.01;
        f1 = LHS_DRE_L(P(:, :, i), E(:, :, i), A(:, :, i), C', Q2, Q1);
        f2 = LHS_DRE_L(P(:, :, i)+f1/2*h, E(:, :, i), A(:, :, i), C', Q2, Q1);
        f3 = LHS_DRE_L(P(:, :, i)+f2/2*h, E(:, :, i), A(:, :, i), C', Q2, Q1);
        f4 = LHS_DRE_L(P(:, :, i)+f3*h, E(:, :, i), A(:, :, i), C', Q2, Q1);
        P(:, :, i+1) = P(:, :, i) + h*(f1/6+(f2+f3)/3+f4/6);
end

% Solve for optimal K and L
for i=1:size(x_k,2)
K(:,:,i) = -R^-1*B'*X(:,:,i);
L(:,:,i) = -P(:,:,i)*C'*Q2^-1;     
end
...
x_k_6 = x_k(1:6,:);
x0=x_k_6(:,end);    %initialize x0 with initial swing up value

%find LQR x values
[x,xsim] = RK4_x(A,B,E,x0,x_k_6,u_k,K);

%find LQG x values
x0 = [0 0 0 0 0 0]';
[x_hat,x_hat_sim]=RK4_xhat(A,B,C,E,x0,x_k_6,u_k,K,L);


%% FUNCTIONS USED FROM BEWLEY GITHUB
function [u_k,x_k]=Example_22_2(T,u_k)
s.h=0.01; s.N=T/s.h; s.mc=10; t=[0:s.N]*s.h;               % STEP 0: initialize simulation
s.m1=1; s.L1=1;    s.ell1=s.L1; s.I1=s.m1*s.ell1^2/3; % system, & derived parameters
s.m2=0.5; s.L2=0.5;  s.ell2=s.L2; s.I2=s.m2*s.ell2^2/3; alpha=0.1;
s.B=[0; 0; 0; 1; 0; 0]; s.Q=diag([1 1 1 1 1 1]); s.R=0; s.QT=diag([5 20 30 .1 5 7]);
if nargin<2, u_k=zeros(s.N+1,1); end, s.x0=[0; pi; pi; 0; 0; 0]; x_k(1:6,1)=s.x0; res=0;
...
end, end % function Example_22_2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function D=DRE(A,B,X,Q,R);  E=Compute_E(x,s); N=Compute_N(x,u,s); R=E\N;
end % function RHS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function dX=RHS_DRE_K(X,E,A,B,R,Q);
% keyboard
% dX = - (E')^-1 \ A' X - X * A * (E)^-1 + X * B * R^-1 * B' * X - (E')^-1 * Q * E^-1;
dX = E'\(-A'*X*E - E'*X*A + E'*X*B*R^-1*B'*X*E+Q)\E;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function dP=LHS_DRE_L(P,E,A,C,Q2,Q1);
% keyboard
% dX = - (E')^-1 \ A' X - X * A * (E)^-1 + X * B * R^-1 * B' * X - (E')^-1 * Q * E^-1;
dP = E'\(A'*P*E + E'*P*A - E'*P*C'*Q2^-1*C'*P*E+Q1)\E;
end
